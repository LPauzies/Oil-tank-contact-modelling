...
public class ContactModellingApplet extends JApplet {
    ...
    private Vector<Map<String, CustomSpinner>> vectorMapTCNameCustomeSpinner = new Vector<>();
    ...
    private void setCustomSpinners() {
    	for (Iterator<TypeCouche> iterator = types.iterator(); iterator.hasNext();) {
			TypeCouche tc = (TypeCouche) iterator.next();
			Map<String, CustomSpinner> map = tc.getCustomSpinners();
			vectorMapTCNameCustomeSpinner.addElement(map);
		}
    }
    ...
    public void reset() {
	    ...
	    gc.TypeCouches.elementAt(0).setNom(nomCoucheTopRock.getText());
	    gc.TypeCouches.elementAt(1).setNom(nomCoucheFluid1.getText());
	    gc.TypeCouches.elementAt(2).setNom(nomCoucheFluid2.getText());
	    gc.TypeCouches.elementAt(3).setNom(nomCoucheBaseRock.getText());
    ...
    public void initInterface () {        
		...
		*/
	    ...
	    final ChangeListener clrho = new ChangeListener () {
	        public void stateChanged(ChangeEvent e) {
	            ...
	            if (obj == rho0) gc.setRho(0,rho0.getValeur());
	            else if (obj == rho1) gc.setRho(1,rho1.getValeur());
	            else if (obj == rho2) gc.setRho(2,rho2.getValeur());
	            else if (obj == rho3) gc.setRho(3,rho3.getValeur());
	            else  throw new IllegalArgumentException("Objet inconnu");
	            gc.checkModeldimension();
	            gc.getModel().ZContact[0]=(gc.getModel().ZM/2d)+(double) GOContact.getValue();
	    		gc.getModel().ZContact[1]=(gc.getModel().ZM/2d)+(double) OWContact.getValue();
	            gc.calculCourbes();
	  ...
	  //On change Vp CustomSpinner do this :
